...
(function () {
  ...
  var L = window.Ledger;
  var STORAGE_DATA = 'gagyebu.v2.data';
  var STORAGE_DATA_V1 = 'gagyebu.v1.data';
  ...
  var state = {
    transactions: [],
    imports: [],   // 가져오기 보고서 기록 (오래된 것부터, 마지막이 최근)
    month: 'all'
  };
  ...
  var els = {
    ...
    statRefundLabel: $('stat-refund-label'),
  ...
  function loadState() {
    ...
    var saved = null;
    if (raw) {
      try { saved = JSON.parse(raw); } catch (e) { storageRemove(STORAGE_DATA); }
    }
    if (!saved) {
      // 1라운드 저장 형식(v1) 이전
      var rawV1 = storageGet(STORAGE_DATA_V1);
      if (rawV1) {
        try {
          var v1 = JSON.parse(rawV1);
          if (v1 && Array.isArray(v1.transactions)) {
            saved = {
              transactions: v1.transactions.map(function (t) {
                var c = {};
                Object.keys(t).forEach(function (k) { if (k !== 'duplicate') c[k] = t[k]; });
                c.source = v1.fileName || '';
                return c;
              }),
              imports: [{
                fileName: v1.fileName || '', importedAt: v1.importedAt || '',
                rowCount: v1.transactions.length, addedCount: v1.transactions.length,
                addedIds: [], duplicates: [], refunds: [], skipped: v1.skipped || [],
                hasTypeColumn: v1.hasTypeColumn !== false, hasIdColumn: true
              }]
            };
          }
        } catch (e) { /* 무시 */ }
        storageRemove(STORAGE_DATA_V1);
      }
    }
  ...
  function lastReport() {
    return state.imports.length ? state.imports[state.imports.length - 1] : null;
  }
  ...
  function handleFile(file) {
    ...
    readFile(file, function (text) {
      var parsed;
      try {
        parsed = L.parseTransactions(text);
      } catch (err) {
        showUploadError(err, file.name);
        return;
      }
      var result = L.mergeTransactions(state.transactions, parsed, {
        fileName: file.name,
        importedAt: new Date().toISOString()
      });
      state.transactions = result.transactions;
      state.imports.push(result.report);
      if (state.imports.length > MAX_HISTORY) state.imports = state.imports.slice(-MAX_HISTORY);
      ...
      hideUploadError();
      render();
      showToast(toastMessage(result.report), 4500);
      window.scrollTo(0, 0);
    }, function (err) {
      showUploadError(err, file.name);
    });
  ...
  function toastMessage(r) {
    var parts = [r.fileName + ': ' + r.addedCount + '건 추가'];
    if (r.duplicates.length) parts.push('중복 ' + r.duplicates.length + '건 제외');
    if (r.refunds.length) parts.push('환불 ' + r.refunds.length + '건 반영');
    if (r.skipped.length) parts.push(r.skipped.length + '행 건너뜀');
    return parts.join(' · ');
  }
  ...
  function render() {
    ...
    var files = [];
    state.imports.forEach(function (r) { if (r.fileName && files.indexOf(r.fileName) === -1) files.push(r.fileName); });
    els.fileChip.textContent = files.length > 1 ? files.length + '개 파일 병합' : (files[0] || '불러온 데이터');
    els.fileChip.title = files.join(', ');
  ...
  function renderWarnings() {
    ...
    var r = lastReport();
    var parts = [];
    if (r && r.skipped && r.skipped.length > 0) {
      var examples = r.skipped.slice(0, 4).map(function (s) {
        return s.line + '행 (' + s.reasons.join(', ') + ')';
      }).join(', ');
      parts.push(r.fileName + ': 형식이 맞지 않아 건너뛴 행 ' + r.skipped.length + '개: ' + examples + (r.skipped.length > 4 ? ' 외' : ''));
    }
    if (r && r.hasTypeColumn === false) {
      parts.push(r.fileName + ': type 열이 없어 모든 거래를 지출로 계산했습니다.');
    }
    if (r && r.hasIdColumn === false) {
      parts.push(r.fileName + ': transaction_id 열이 없어 날짜·가맹점·분류·금액으로 ID를 만들었습니다. 같은 파일을 다시 올리면 중복으로 제외됩니다.');
    }
  ...
  // 최근 가져오기 결과: 추가·중복 제외·환불 반영 내역
  function renderReport() {
    var r = lastReport();
    if (!r) { els.reportCard.hidden = true; return; }
    els.reportCard.hidden = false;
    els.reportFile.textContent = r.fileName + (r.importedAt ? ' · ' + new Date(r.importedAt).toLocaleString('ko-KR') : '');

    var summary = els.reportSummary;
    summary.innerHTML = '';
    var existingDups = r.duplicates.filter(function (d) { return d.reason === 'existing'; }).length;
    var fileDups = r.duplicates.length - existingDups;
    var items = [
      { label: '읽은 행', value: r.rowCount + '행' },
      { label: '새로 추가', value: r.addedCount + '건', strong: true },
      { label: '중복 제외', value: r.duplicates.length + '건' +
          (r.duplicates.length ? ' (기존과 같은 ID ' + existingDups + ', 파일 안 반복 ' + fileDups + ')' : '') },
      { label: '환불 반영', value: r.refunds.length + '건' }
    ];
    if (r.skipped.length) items.push({ label: '형식 오류로 건너뜀', value: r.skipped.length + '행' });
    items.forEach(function (it) {
      var li = el('li', 'report__item');
      li.appendChild(el('span', 'report__label', it.label));
      li.appendChild(el('span', 'report__value' + (it.strong ? ' report__value--strong' : ''), it.value));
      summary.appendChild(li);
    });
    if (r.addedCount === 0 && r.duplicates.length > 0) {
      var note = el('li', 'report__note', '이 파일의 거래는 모두 이미 반영되어 있어 거래 수와 합계가 바뀌지 않았습니다.');
      summary.appendChild(note);
    }

    var details = els.reportDetails;
    details.innerHTML = '';

    if (r.duplicates.length) {
      var d1 = el('details', 'report__details');
      d1.appendChild(el('summary', null, '중복으로 제외한 행 ' + r.duplicates.length + '건 보기'));
      var ul1 = el('ul', 'report__list');
      r.duplicates.forEach(function (d) {
        var li = el('li', 'report__row');
        li.appendChild(el('span', 'report__line', d.line + '행'));
        li.appendChild(el('span', 'report__id', d.id));
        li.appendChild(el('span', 'report__desc', d.date + ' ' + d.merchant + ' · ' + d.category));
        li.appendChild(el('span', 'report__amt', L.formatKrw(d.amount)));
        li.appendChild(el('span', 'badge', d.reason === 'existing' ? '이미 있는 ID' : '파일 안 반복'));
        ul1.appendChild(li);
      });
      d1.appendChild(ul1);
    ...
    if (r.refunds.length) {
      var d2 = el('details', 'report__details');
      d2.appendChild(el('summary', null, '반영한 환불 ' + r.refunds.length + '건 보기'));
      var ul2 = el('ul', 'report__list');
      r.refunds.forEach(function (f) {
        var li = el('li', 'report__row');
        li.appendChild(el('span', 'report__id', f.id));
        li.appendChild(el('span', 'report__desc', f.date + ' ' + f.merchant + ' · ' + f.category));
        li.appendChild(el('span', 'report__amt report__amt--refund', L.formatKrw(f.amount)));
        var ref;
        if (!f.originalId) ref = el('span', 'badge', '원거래 정보 없음');
        else if (f.originalFound) ref = el('span', 'badge badge--refund', '원거래 ' + f.originalId + ' · ' + f.originalDate + ' ' + L.formatKrw(f.originalAmount));
        else ref = el('span', 'badge badge--warn', '원거래 ' + f.originalId + ' 를 찾지 못함');
        li.appendChild(ref);
        ul2.appendChild(li);
      });
      d2.appendChild(ul2);
    ...
    if (state.imports.length > 1) {
      var d3 = el('details', 'report__details');
      d3.appendChild(el('summary', null, '가져온 파일 기록 ' + state.imports.length + '건'));
      var ul3 = el('ul', 'report__list');
      state.imports.slice().reverse().forEach(function (h) {
        var li = el('li', 'report__row');
        li.appendChild(el('span', 'report__id', h.fileName));
        li.appendChild(el('span', 'report__desc', (h.importedAt ? new Date(h.importedAt).toLocaleString('ko-KR') + ' · ' : '') +
          '추가 ' + h.addedCount + '건, 중복 ' + h.duplicates.length + '건, 환불 ' + h.refunds.length + '건'));
        ul3.appendChild(li);
      });
      d3.appendChild(ul3);
  ...
  function renderStats(summary) {
    ...
    els.statRefund.textContent = L.formatKrw(summary.refund);
    els.statRefundLabel.textContent = '환불 합계' + (summary.refundCount ? ' · ' + summary.refundCount + '건' : '');
    els.statCount.textContent = L.formatNumber(summary.count) + '건';

    var sub = [];
    if (summary.refundCount > 0) sub.push('지출 ' + L.formatKrw(summary.expense) + ' + 환불 ' + L.formatKrw(summary.refund));
    ...
    var items = byMonth.slice().reverse().map(function (b) {
      ...
      return {
        ...
        title: L.formatMonth(b.month) + ' 순지출 ' + L.formatKrw(b.net) + ' · ' + b.count + '건' +
          (b.refundCount ? ' (환불 ' + b.refundCount + '건 ' + L.formatKrw(b.refund) + ' 포함)' : '')
      };
    });
  ...
  function renderTable(filtered) {
    ...
    var refundMap = L.refundsByOriginal(state.transactions);
    var files = [];
    state.imports.forEach(function (r) { if (r.fileName && files.indexOf(r.fileName) === -1) files.push(r.fileName); });
    els.txCount.textContent = rows.length + '건';
    els.txHint.textContent = (files.length ? files.join(' + ') + ' · ' : '') + '최근 거래부터';
    ...
    rows.forEach(function (t) {
      ...
      if (t.type === 'refund') tr.className = 'tx--refund';
      ...
      if (t.type === 'refund') {
        var rb = el('span', 'badge badge--refund', t.originalId ? '환불 · 원거래 ' + t.originalId : '환불');
        rb.title = t.originalId ? '거래 ' + t.originalId + ' 의 환불입니다.' : '환불 거래입니다.';
        tdMerchant.appendChild(rb);
      } else if (refundMap[t.id]) {
        var ids = refundMap[t.id].map(function (f) { return f.id; }).join(', ');
        var ob = el('span', 'badge', '환불됨 · ' + ids);
        ob.title = '이 거래를 참조하는 환불이 있습니다: ' + ids;
        tdMerchant.appendChild(ob);
      }
      var showId = t.id && !/^auto-/.test(t.id);
      var meta = el('div', 'tx__meta', t.category + (showId ? ' · ' + t.id : ''));
      tdMerchant.appendChild(meta);

      var tdCat = document.createElement('td');
      tdCat.className = 'tx__category';
      tdCat.textContent = t.category;
      if (showId) tdCat.appendChild(el('span', 'tx__id', t.id));
      ...
      if (t.type === 'expense') {
        ...
        tdAmt.textContent = L.formatKrw(t.amount);
        var eb = el('span', 'badge', (L.TYPE_LABELS[t.type] || t.rawType || '기타') + ' · 제외');
  ...
  function bindEvents() {
    ...
    // 페이지 다른 곳에 떨어뜨렸을 때 브라우저가 파일을 열어버리지 않도록. 대시보드에서는 어디에 놓아도 병합한다.
    ...
    els.btnClear.addEventListener('click', function () {
      if (!window.confirm('불러온 데이터와 가져오기 기록을 이 브라우저에서 지울까요? 원본 CSV 파일은 그대로 남습니다.')) return;
