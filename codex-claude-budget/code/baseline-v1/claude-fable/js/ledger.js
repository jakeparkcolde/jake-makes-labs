...
(function (root, factory) {
  ...
  function importCsv(text) {
    ...
    for (var r = 1; r < rows.length; r++) {
      ...
      var id = get('id');
      if (!id || usedIds[id]) id = (id ? id + '#' : 'row') + lineNo;
      usedIds[id] = true;
      transactions.push({
        id: id,
        date: date,
  ...
  // 같은 날짜·가맹점·금액·유형이 두 번 이상 나오면 중복 의심 표시 (합계에서 제외하지는 않음)
  function markDuplicates(transactions) {
    var seen = {};
    transactions.forEach(function (t) {
      var key = [t.date, t.merchant, t.amount, t.type].join('|');
      seen[key] = (seen[key] || 0) + 1;
    });
    transactions.forEach(function (t) {
      var key = [t.date, t.merchant, t.amount, t.type].join('|');
      t.duplicate = seen[key] > 1;
    });
    return transactions;
  }

  // ---------- 집계 ----------
  // 순지출 기여액: 지출은 +금액, 환불은 -|금액|, 수입·이체·기타는 0 (순지출에서 제외)
  ...
  function emptyBucket(key, keyName) {
    var b = { net: 0, expense: 0, refund: 0, count: 0, excluded: 0 };
    b[keyName] = key;
    return b;
  }
  ...
  function accumulate(bucket, t) {
    ...
    else if (t.type === 'refund') bucket.refund += Math.abs(t.amount);
